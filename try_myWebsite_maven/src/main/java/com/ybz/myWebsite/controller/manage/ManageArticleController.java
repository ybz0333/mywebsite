...
public class ManageArticleController extends BaseController{
	...
	public String queryAllArticles(Model model, ArticleSearchVo articleSearchVo, Date chosenCreateTime) throws Exception{
		...
		return "manage/blog/manageArticle";
	}
	
	//编辑新文章
	@RequestMapping("/editNewArticle")
	public String editNewArticle(HttpServletRequest request, Model model) throws Exception {
		...
		// 获取文章配图列表
		File articleImgDir = new File(request.getSession().getServletContext()
				.getRealPath("/img/article_picture/"));
		String[] articleImgNames = articleImgDir.list();
		//System.out.println(Arrays.toString(articleImgNames));
		model.addAttribute("articleImgNames", articleImgNames);
		return "manage/blog/editArticle";
	}
	
	//添加新文章到数据库
	@RequestMapping("/addNewArticle")
	public @ResponseBody String addNewArticle(ArticleVo articleVo, String newArticleTypeText) throws Exception {
		//时间为空则设置为当前时间
		...
		//若文章类型中没有类型1（所有文章），则添加类型1
		ArticleType articleTypeTemp = new ArticleType();
		articleTypeTemp.setId(1);
		List<ArticleType> articleTypeList = articleVo.getArticleTypeList();
		if(!articleTypeList.contains(articleTypeTemp)) {
			articleTypeList.add(articleTypeTemp);
		}
	...
	public String editArticle(HttpServletRequest request, Model model, Integer id) throws Exception{
		if(id == null) {
	...
	public @ResponseBody String updateArticle(ArticleVo articleVo, String newArticleTypeText) throws Exception {
	...
	public ModelAndView changeState(Integer id, Integer stateChangeTo, Integer pageNum) throws Exception {
		...
		return new ModelAndView("redirect:/manage/blog/manageArticle.action",modelMap);
	}
