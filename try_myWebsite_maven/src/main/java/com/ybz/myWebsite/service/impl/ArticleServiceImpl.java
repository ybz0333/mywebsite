...
public class ArticleServiceImpl implements ArticleService{
	...
	public Pager<ArticleVo> findArticleSearchVoWithPager(ArticleSearchVo articleSearchVo)
		...
		//查询条件中有文章类型的情况比较特殊，需要再通过文章id查询对应的文章类型列表，补充到原查询结果中
		if(hasArticleType == true) {
			for(ArticleVo articleVo : articleVoList) {
				List<ArticleType> articleTypeList = articleTypeMapper
						.findArticleTypesByArticleId(articleVo.getId());
				articleVo.setArticleTypeList(articleTypeList);
			}
		}
